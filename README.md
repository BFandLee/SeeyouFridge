...
### 문제 상황2 : Hugging face spaces 무료버전 특성상 3일동안 서버 접속 없을 시 수면 상태에 돌입하여 앱이 3일동안 미접속 시 서버를 불러오지 못하여 timeout걸리는 현상 발생

#### 해결방안
UptimeRobot 사용 --> uptimeRobot은 5~10분 간격으로 HTTP 신호를 보냄 --> 계속해서 서버에 신호가 가므로써 서버가 꺼지지 않도록 함

## 설치 및 실행
app_debug.apk 파일을 스마트폰에 다운로드하여 실행
